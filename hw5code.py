...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    split_positions = np.where(feature_sorted[1:] != feature_sorted[:-1])[0]
    if len(split_positions) == 0:
        return np.array([]), np.array([]), None, None
    thresholds = (feature_sorted[split_positions] + feature_sorted[split_positions + 1]) / 2

    cumsum_ones = np.cumsum(target_sorted)
    left_size = split_positions + 1
    right_size = n - left_size
    left_ones = cumsum_ones[split_positions]
    right_ones = cumsum_ones[-1] - left_ones
    left_zeros = left_size - left_ones
    right_zeros = right_size - right_ones
    p1_left = left_ones / left_size
    p0_left = left_zeros / left_size
    p1_right = right_ones / right_size
    p0_right = right_zeros / right_size
    ...
    def _fit_node(self, sub_X, sub_y, node, depth=0):
        ...
        for feature in range(sub_X.shape[1]):
            ...
            if self._min_samples_leaf is not None:
                ...
                if mask.sum() < self._min_samples_leaf or (~mask).sum() < self._min_samples_leaf:
                    continue
